...
def benchmark(
    ...
    output_w = torch.empty(batch_size, output_size, dtype=torch.float32, device=device)
    output_b = torch.empty(batch_size, output_size, dtype=torch.float32, device=device)

    kernel = make_sparse_input_linear_forward_kernel(max_active_indices, output_size)

    # Warmup (triggers autotune + compilation)
    for _ in range(warmup):
        kernel(white_indices, weight, bias, output_w)
        kernel(black_indices, weight, bias, output_b)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Benchmark sparse input linear forward kernel with real data"
    )
